...
vec3 CalcDiffuse(vec3 worldPos, vec3 normal, Light light) {
	vec3 toLight = light.Position.xyz - worldPos;
	float dist = length(toLight);
	toLight = normalize(toLight);
	float diffuseFactor = max(dot(normal, toLight), 0);
...
vec3 CalcSpecular(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess) {
	vec3 toLight = light.Position.xyz - worldPos;
	float dist = length(toLight);
	toLight = normalize(toLight);
	vec3 halfDir     = normalize(toLight + viewDir);
...
vec3 CalcAllLightContributionSpecularOnly(vec3 worldPos, vec3 normal, vec3 camPos, float shininess) {
	// Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcSpecular(worldPos, normal, viewDir, Lights[ix], shininess);
	}

	return lightAccumulation;
}

vec3 CalcAllLightContributionDiffuseOnly(vec3 worldPos, vec3 normal, vec3 camPos) {
	//CalcDiffuse(vec3 worldPos, vec3 normal, Light light)
	// Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += CalcDiffuse(worldPos, normal, Lights[ix]);
	}
